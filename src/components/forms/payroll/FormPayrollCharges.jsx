...
export default function FormPayrollCharges() {
  ...
  useEffect(() => {
    const realizarPeticion = async () => {
      ...
      setPositions(respuesta3);
      const respuesta4 = await peticionGet(
        "http://localhost:3000/api/employees/all",
        "GET"
      );
      setEmployees(respuesta4);
    };
  ...
  const handleSubmit = async (e) => {
    ...
    const respuesta = await peticionPost(
      `http://localhost:3000/api/payrolls/generate/position/${values.position}`,
      "POST",
      values
    );
      ...
      {info === null ? (
        ...
        >
          {({ loading, url, error, blob }) =>
            loading ? (
              <button className="text-center w-full text-xl">
                Loading Document ...
              </button>
            ) : (
              <button className="text-center w-full text-xl">
